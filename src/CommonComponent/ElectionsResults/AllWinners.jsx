...
import {
  ...
  selectWinnerCandiPage,
  handleWinnerCandiPages,
  selectTotalWinnerCandidates,
  getAllWinnerCandidatesAsync,
} from "./CommonSlice";
import NavBar from "../Navigations/HomePageMenu";
...
const CandidatePage = () => {
  ...
  const candidates = useSelector(selectWinnerCandidates);
  // console.log(candidates);
  const candidatestotalcount = useSelector(selectTotalWinnerCandidates);
  //role: "voter", constituency: "Lanja"
  /////// Filter Settings
  const totalPages = Math.ceil(candidatestotalcount / ITEMS_PER_PAGE);
  const [filter, setFilter] = useState({});
  const pages = useSelector(selectWinnerCandiPage);
  const handlePage = (pagea) => {
    dispatch(handleWinnerCandiPages(pagea));
  };
  ...
  useEffect(() => {
    ...
    dispatch(getAllWinnerCandidatesAsync({ pagination, filter }));
            ...
            <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
              <div className="flex items-baseline justify-between border-b border-gray-200 pb-6 pt-3">
                <h3 className="text-2xl font-bold mb-4">
                  All Winner Candidate
                </h3>
                <div className="mb-4 font-bold">
                  Total Winner Candidate Count : {candidatestotalcount}
                </div>
