...
const filters = [
  ...
  },
  {
    ...
    options: [
      ...
      { value: "minner", label: "minner", checked: false },
    ],
  },
];
const MinnerPage = () => {
  ...
  /////// Filter Settings
  const totalPages = Math.ceil(MinnerTotalCount / ITEMS_PER_PAGE);
  const [filter, setFilter] = useState({});
  const pages = useSelector(selectMinerPage);
  const handlePage = (pagea) => {
    dispatch(handleMinerPages(pagea));
  };
                  ...
                  <div className="lg:col-span-3">
                    {
                      <ViewList
                        minner={minner}
                        MinnerTotalCount={MinnerTotalCount}
                        dispatch={dispatch}
                      />
                    }
                  </div>
...
function ViewList({ minner, MinnerTotalCount, dispatch }) {
  return (
            ...
            >
              <div className="flex items-center mb-4">
                <img
                  ...
                  alt={miner.name}
                  className="w-16 h-16 rounded-full object-cover mb-1 mr-2"
                />
                <p className="font-semibold">{miner.name}</p>
              </div>
              <p>
                Username:{" "}
                {miner.username ? miner.username : "Username not available"}
              </p>
              <p>Email: {miner.email ? miner.email : "Email not available"}</p>
              <p>Role: {miner.role ? miner.role : "Role not available"}</p>
              <p>
                Minner ID:{" "}
                {miner.MinnerID ? miner.MinnerID : "Minner ID not available"}
              </p>
              <p>
                Region : {miner.region ? miner.region : "Region not available"}
              </p>
              <div className="flex justify-end mt-4">
                <Link to={`/UpdateMinner/${miner.id}`}>
                ...
                <button
                  onClick={() => {
                    dispatch(deleteMinnerAsync(miner.id));
                  }}
                >
                  <FaTrash />
                </button>
